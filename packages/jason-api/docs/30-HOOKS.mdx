...
you options object should include the `action` property with the action to execute
(created by `jasonApiRequest()`). The dependency list is an array of variables who, when changed,
will cause your request to change. It will return a tuple containing info aboute the request and a
callback to initiate the request.
...
<Parameters
    parameters={[
        ...
        },
        {
            name: 'false',
            required: true,
            description: 'An array of all values from the Component scope that are used as part of the request.'
        }
    ]}
/>
