...
import { defaultState, ArticleResource } from '../../__mocks__';

const state = { jasonApi: defaultState };

describe('getResourceObject', () => {
    it('should return an resource object', () => {
        const resourceObject = getResourceObject<ArticleResource>(
            state,
            'article',
            '1'
        )!;
...
describe('getResourceObjects', () => {
    it('should return all resource objects', () => {
        const results = getResourceObjects<ArticleResource>(state, 'comments');
    ...
    it('should return an empty array if the resource objects do not exist', () => {
        const invalidIdsResult = getResourceObjects<ArticleResource>(
            state,
            'comments',
            ['666', '777']
        );
        ...
        const invalidTypeResult = getResourceObjects<ArticleResource>(
            state,
            'spicyboys'
        );
...
describe('getCachedQuery', () => {
    it('should get a cached query', () => {
        const result = getCachedQuery<ArticleResource[]>(state, 'cacheKey');
    ...
    it('should fetch and expand a cached query', () => {
        const result = getCachedQuery<ArticleResource[]>(
            state,
            'cacheKey',
            true
        )!;
